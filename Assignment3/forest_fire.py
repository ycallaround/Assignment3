...
def find_flames(filename):
    ...
    for pixel in image:
        if is_suf_red(pixel):
            # checking if sufficient red, if so, emphasizing the red
            pixel.red = 255
            pixel.green = 0
            pixel.blue = 0
        else:
            pixel = grey_scale_it(pixel)
            # calling the function that convert the pixel to gray scale
    return image
...
def is_suf_red(pixel):
    # check if the pixel we received is sufficiently red
    if (pixel.red > ((pixel.red + pixel.blue + pixel.green)/3)) or (pixel.red == ((pixel.red + pixel.blue + pixel.green)/3)) :
        return True
    else:
        return False

def grey_scale_it(pixel):
    pixel.green  = ((pixel.red + pixel.blue + pixel.green)/3)
    pixel.red = ((pixel.red + pixel.blue + pixel.green) / 3)
    pixel.blue = ((pixel.red + pixel.blue + pixel.green) / 3)
    return (pixel)
